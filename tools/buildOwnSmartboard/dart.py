...
class GamePage(tk.Frame):
    def __init__(self, master_page, num_games, num_players, player_names, *args, **kwargs):
        ...
        self.dartPinArray_outputs = [5,6,12,13,16,17,18,19]
        self.dartPinArray_inputs = [20,21,22,23,24,25,26,27]

        self.dartMatrix = np.zeros((len(self.dartPinArray_outputs),len(self.dartPinArray_inputs)))
        ...
        # Spiel Buttons
        self.nextplayer_button = tk.Button(self.game_page, text="Next Player", command=self.next_player, font=('arial', 15, 'bold'))
        self.nextplayer_button.grid(row=self.num_players+4, column=0, padx=5, pady=1)
        self.keintreffer_button = tk.Button(self.game_page, text="kein treffer", command=self.keintreffer, font=('arial', 15, 'bold'))
        self.keintreffer_button.grid(row=self.num_players+4, column=4, padx=5, pady=5)
        self.wandtreffer_button = tk.Button(self.game_page, text="Wandtreffer", command=self.wandtreffer, font=('arial', 15, 'bold'))
        self.wandtreffer_button.grid(row=self.num_players+4, column=5, padx=5, pady=5)
        self.wurfreset_button = tk.Button(self.game_page, text="Wurf reset", command=self.wurfreset, font=('arial', 15, 'bold'))
    ...
    def gpio_setup(self):
        # Set Outputs
        for pin in self.dartPinArray_outputs:
            GPIO.setup(pin, GPIO.OUT)

        # Set Inputs
        for pin in self.dartPinArray_inputs:
            GPIO.setup(pin, GPIO.IN, pull_up_down=GPIO.PUD_UP)

    def next_player(self):
        print("Button Next Player")
    ...
    def warte_auf_wurf(self):
        ...
        while self.condition_quit == False:
            ...
            for zeile in range(len(self.dartPinArray_outputs)):
                ...
                GPIO.output(self.dartPinArray_outputs[zeile], GPIO.LOW)

                for spalte in range(len(self.dartPinArray_inputs)):
                    # Lese GPIO inputs ein und schreibe 1 oder 0 in dartMatrix
                    self.dartMatrix[zeile][spalte] = GPIO.input(self.dartPinArray_inputs[spalte])

                    # Wenn GPIO input auf LOW, dann wurde das  Feld getroffen
                    if (self.dartMatrix[zeile][spalte] == 0):
                        print("Wurf detektiert")
                        ergebnis = self.werteMatrix[zeile][spalte]
                        return ergebnis
        return 999
        #NUR FUER TEST
    ...
    def play_sound(self, str):
        ...
        #strafe = pygame.mixer.Sound('./sounds/strafe.mp3')
